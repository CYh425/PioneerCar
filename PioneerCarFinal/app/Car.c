...
static HAL_StatusTypeDef i2c_tx(uint8_t add, uint8_t mem, uint8_t data) {
	return HAL_I2C_Mem_Write(&hi2c2, add, mem, I2C_MEMADD_SIZE_8BIT, &data, 1, 1000);
}
static HAL_StatusTypeDef i2c_rx(uint8_t add, uint8_t mem, uint8_t *data) {
	return HAL_I2C_Mem_Read(&hi2c2, add, mem, I2C_MEMADD_SIZE_8BIT, data, 1, 1000);
}
static HAL_StatusTypeDef WheelInit(WheelType *_Wheel,
                                   uint8_t ForwardPin,
                                   uint8_t BackwardPin,
                                   TIM_HandleTypeDef *Timmer) {
	_Wheel->Motor.ForwardPin = ForwardPin;
	_Wheel->Motor.BackwardPin = BackwardPin;
	_Wheel->Encoder.Timer = Timmer;
...
HAL_StatusTypeDef CarInit(CarType *_Car,
                                 TIM_HandleTypeDef *LeFr,
                                 TIM_HandleTypeDef *RiFr,
                                 TIM_HandleTypeDef *LeRe,
                                 TIM_HandleTypeDef *RiRe) {
	WheelInit(&_Car->LeftFront, MotorLeftFrontForward_Pin, MotorLeftFrontBackward_Pin, LeFr);
	WheelInit(&_Car->RightFront, MotorRightFrontForward_Pin, MotorRightFrontBackward_Pin, RiFr);
	WheelInit(&_Car->LeftRear, MotorLeftRearForward_Pin, MotorLeftRearBackward_Pin, LeRe);
	WheelInit(&_Car->RightRear, MotorRightRearForward_Pin, MotorRightRearBackward_Pin, RiRe);
	...
	if (PAL_PCA9685_IsSleep(&PCA9685)) {
		PAL_PCA9685_Sleep(&PCA9685, false);
		PAL_PCA9685_SetOutput(&PCA9685, 0, 1.0f, 0);

	}
	return HAL_OK;
}
HAL_StatusTypeDef WheelSetRadVelocity(WheelType *_Wheel, float RadVelocity) {
	_Wheel->Motor.RadVelocity = RadVelocity;
	return HAL_OK;
}
...
static HAL_StatusTypeDef MotorOutSpeed(WheelType *_Wheel) {
	...
	if (radVelocity > 0) {
		PAL_PCA9685_SetOutput(&PCA9685, _Wheel->Motor.BackwardPin, 0, 0);
		PAL_PCA9685_SetOutput(&PCA9685,
		                      _Wheel->Motor.ForwardPin,
		                      fabsf(VelocityCurve(radVelocity)),
		                      0);
	} else {
		PAL_PCA9685_SetOutput(&PCA9685, _Wheel->Motor.ForwardPin, 0, 0);
		PAL_PCA9685_SetOutput(&PCA9685,
		                      _Wheel->Motor.BackwardPin,
		                      fabsf(VelocityCurve(radVelocity)),
		                      0);
	}
	return HAL_OK;
...
static HAL_StatusTypeDef WheelRadVelocityLoop(WheelType *_Wheel) {
	_Wheel->Parameter.bias =
		(_Wheel->Motor.RadVelocity - _Wheel->Encoder.RadVelocity) * _Wheel->Parameter.kp;
	_Wheel->Parameter.integral += _Wheel->Parameter.bias * _Wheel->Parameter.ki;
	_Wheel->Parameter.PID =
		_Wheel->Motor.RadVelocity + _Wheel->Parameter.bias + _Wheel->Parameter.integral;
	return HAL_OK;
...
HAL_StatusTypeDef CarUpdate(CarType *_Car) {
	WheelUpdate(&_Car->LeftFront);
	WheelUpdate(&_Car->LeftRear);
	WheelUpdate(&_Car->RightFront);
	WheelUpdate(&_Car->RightRear);
	return HAL_OK;
}
void CarMoveRad(CarType *_Car, float DirectionRad, float Speed) {
	float Xaxis, Yaxis;
	Xaxis = Speed * cosf(DirectionRad);
	Yaxis = Speed * sinf(DirectionRad);
	WheelSetRadVelocity(&_Car->LeftFront, Yaxis - Xaxis);
	WheelSetRadVelocity(&_Car->LeftRear, Yaxis + Xaxis);
	WheelSetRadVelocity(&_Car->RightFront, Yaxis + Xaxis);
	WheelSetRadVelocity(&_Car->RightRear, Yaxis - Xaxis);
	CarUpdate(_Car);
}
void CarMoveInt(CarType *_Car, int X_intensity, int Y_intensity) {
	double Xaxis, Yaxis;
	Xaxis = 1.0 * X_intensity / 850.0;
	Yaxis = 1.0 * Y_intensity / 850.0;
